...
> **本サンプルの実行環境**: `node weak-server.js` は Mac 上の Node プロセス。ブラウザでアクセスする `http://localhost:3001` も Mac の中。攻撃者役・被害者役のブラウザタブも同じ Mac で開いているだけ。**「サーバ」と「クライアント」は同じ Mac の中の別プロセス**で、別マシン間の攻撃を再現してはいない。XSS の本質はアプリコード内のバグなので、この簡略化でも現象は同じに見える。
